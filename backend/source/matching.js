...
// retourne true si la voiture passe les filtres durs
...
function scoreMarque(voiture,prefs){
    const preferees = prefs.marque;
    if (!preferees || preferees.lenght==0)return 1;
    return preferees.includes(voiture.marque) ? 1 : 0.2;

}

function scoreCarburant(voiture,prefs){
    const preferees = prefs.carburant;
    if (!preferees || preferees.lenght==0)return 1;
    return preferees.includes(voiture.carburant) ? 1 : 0;
...
/**
 * @param {Array} voitures - liste brute des voitures (depuis la DB)
 * @param {Object} prefs - preferences utilisateur
 * @param {number} limit - nombre max de resultats
 */
function recommanderVoitures(vowitures, prefs, limit = 20) {
